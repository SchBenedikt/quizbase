    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground selection:bg-primary selection:text-primary-foreground">
      ...
      <main className="flex-grow pt-32">
        <section className="px-6 pt-12 pb-32 max-w-7xl mx-auto grid lg:grid-cols-2 gap-20 items-center">
          <div className="space-y-12">
            ...
            <p className="text-xl md:text-2xl font-bold max-w-xl leading-tight uppercase tracking-tight opacity-80">
              Verwandeln Sie jeden Raum in ein interaktives Erlebnis. Keine Apps, kein Stress. Pure Energie.
            </p>
            ...
            <form onSubmit={handleJoin} className="flex flex-col sm:flex-row gap-4 pt-6">
              ...
              <Button type="submit" disabled={joinCode.length < 6} className="h-20 px-12 rounded-[1.5rem] text-xl font-black bg-foreground text-background border-2 border-foreground hover:bg-transparent hover:text-foreground transition-all group shrink-0 shadow-none">
                JETZT TEILNEHMEN <ArrowRight className="ml-3 h-8 w-8 group-hover:translate-x-2 transition-transform" />
              </Button>
            </form>
          </div>
        ...
        <section id="features" className="bg-foreground text-background py-40 px-6">
          <div className="max-w-7xl mx-auto space-y-32">
            <div className="text-center space-y-6">
              ...
              <p className="text-xs font-black uppercase tracking-widest opacity-40">Entwickelt für maximale Reichweite</p>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                { icon: Cloud, title: "Wortwolken", desc: "Sehen Sie, was Ihre Zielgruppe wirklich denkt." },
                { icon: ListChecks, title: "Umfragen", desc: "Sofortige Ergebnisse. Entscheidungen in Echtzeit." },
                { icon: SlidersHorizontal, title: "Regler", desc: "Hochpräzise Daten für tiefe Einblicke." },
                { icon: Sparkles, title: "KI-Hilfe", desc: "Optimieren Sie Ihre Fragen in Sekundenschnelle." }
              ].map((tool, i) => (
          ...
          <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-32 items-center">
            <div className="space-y-12">
              <h2 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-[0.85]">
                Begeistern <br />
                Sie alle. <br />
                <span className="opacity-20 italic">Überall.</span>
              </h2>
              <p className="text-xl font-bold uppercase opacity-70 leading-tight max-w-lg">
                PopPulse* entfernt alle Barrieren. Keine Registrierung für Teilnehmer – der Mensch steht im Mittelpunkt.
              </p>
              <div className="flex gap-6 items-center p-8 bg-foreground/5 rounded-[1.5rem] border-2 border-foreground/10">
                ...
                <div>
                  <h4 className="text-xl font-black uppercase tracking-tighter">100% Teilhabe</h4>
                  <p className="font-bold opacity-40 uppercase text-xs tracking-widest">Jeder kommt zu Wort</p>
                </div>
              </div>
            </div>
        ...
        <section className="py-60 px-6 text-center bg-foreground text-background">
          <div className="max-w-4xl mx-auto space-y-16">
            ...
            <p className="text-sm font-black uppercase tracking-[0.4em] opacity-40 pt-12">Kein Account nötig. Sofortiger Start.</p>
          </div>
        </section>
      </main>
      ...
      <footer className="py-24 px-6 max-w-7xl mx-auto w-full flex flex-col md:flex-row items-center justify-between gap-12 font-black border-t-2 border-foreground">
        <div className="flex flex-col gap-4">
          ...
          <p className="text-xs opacity-40 uppercase tracking-widest">Globale Echtzeit-Interaktion</p>
        </div>
