    ...
    <div className="space-y-16 pb-48 presenter-ui">
      ...
      <section className="space-y-8">
        <div className="flex items-center gap-4 px-4">
          ...
          <h3 className="text-2xl font-black uppercase tracking-tighter">Color Architecture</h3>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {themes.map((t) => (
            <button
              ...
              className={cn(
                "flex items-center justify-between px-8 py-6 rounded-[2rem] border-4 transition-all hover:scale-[1.02] active:scale-95 group",
            ...
            >
              ...
              <div 
                className="w-8 h-8 rounded-xl border-4 border-primary/20 transition-transform group-hover:rotate-12"
      ...
      <section className="space-y-10">
        ...
        <div className="grid gap-10">
          {questions.map((q, idx) => (
            <Card key={q.id} className="border-8 border-primary rounded-[3.5rem] bg-white overflow-hidden shadow-none transition-all hover:border-primary/80">
              <CardContent className="p-12">
                <div className="flex flex-col lg:flex-row gap-12">
                  <div className="bg-primary text-background w-16 h-16 rounded-[1.5rem] flex items-center justify-center font-black text-3xl shrink-0">
                    {idx + 1}
                  </div>
                  <div className="flex-grow space-y-10">
                    <div className="flex items-center justify-between">
                      <Label className="text-[10px] font-black uppercase tracking-[0.6em] opacity-40">Interactive Element</Label>
                      <span className="px-6 py-2 bg-primary/5 rounded-full text-[10px] font-black uppercase text-primary border-4 border-primary/10">
                        {q.type.replace('-', ' ')}
                      </span>
                    </div>
                    ...
                    <div className="flex gap-6">
                      <Input 
                        ...
                        className="text-3xl font-black h-24 border-4 border-primary/10 bg-white rounded-[2.5rem] px-10 focus-visible:ring-0 placeholder:opacity-20 shadow-none"
                      />
                      <Button variant="ghost" size="icon" onClick={() => removeQuestion(q.id)} className="text-primary hover:bg-destructive hover:text-white h-24 w-24 rounded-[2.5rem] border-4 border-primary/10 transition-all">
                    ...
                    {q.type === 'multiple-choice' && q.options && (
                      <div className="grid gap-4 pl-10 border-l-8 border-primary/10 mt-10">
                        <Label className="text-[8px] font-black uppercase tracking-[0.5em] opacity-40 mb-2">Options Architecture</Label>
                        {q.options.map((opt, oIdx) => (
                          <div key={oIdx} className="flex gap-4 items-center group">
                            ...
                            <Input 
                              ...
                              className="h-16 border-4 border-primary/10 bg-primary/5 rounded-2xl px-8 focus-visible:ring-0 font-black text-lg shadow-none"
                            />
                            {q.options!.length > 2 && (
                               <Button variant="ghost" size="icon" onClick={() => {
                                 ...
                                 <Trash2 className="h-5 w-5 opacity-40 hover:opacity-100" />
                               </Button>
                            )}
                          </div>
                        ))}
                        <Button 
                          ...
                          className="rounded-[1.5rem] border-4 border-dashed border-primary/30 font-black h-16 uppercase tracking-widest text-xs hover:border-primary shadow-none mt-2"
                        >
                          <Plus className="mr-3 h-5 w-5" /> Expand Choice Array
                        </Button>
                      </div>
                    )}
                  </div>
      ...
      <div className="fixed bottom-12 left-1/2 -translate-x-1/2 bg-primary p-4 rounded-[4rem] flex items-center gap-4 border-8 border-primary z-50">
        ...
        <Button onClick={() => addQuestion('word-cloud')} className="rounded-full h-16 px-8 gap-3 bg-transparent hover:bg-white/10 text-white border-none font-black uppercase text-[10px] tracking-widest shadow-none">
          <Cloud className="h-6 w-6" /> Cloud
        </Button>
        <div className="w-1.5 h-10 bg-white/20 rounded-full" />
        <Button onClick={() => addQuestion('open-text')} className="rounded-full h-16 px-8 gap-3 bg-transparent hover:bg-white/10 text-white border-none font-black uppercase text-[10px] tracking-widest shadow-none">
          <MessageSquare className="h-6 w-6" /> Open
        </Button>
        <div className="w-1.5 h-10 bg-white/20 rounded-full" />
        <Button onClick={() => addQuestion('slider')} className="rounded-full h-16 px-8 gap-3 bg-transparent hover:bg-white/10 text-white border-none font-black uppercase text-[10px] tracking-widest shadow-none">
