...
export function PollCreator({ onSave, initialQuestions = [] }: PollCreatorProps) {
  ...
  return (
    <div className="space-y-12 pb-48 text-primary">
      ...
      {questions.length === 0 ? (
        <Card className="border-4 border-dashed border-primary/30 bg-primary/5 py-32 text-center rounded-[4rem]">
        ...
        <div className="grid gap-8">
          {questions.map((q, idx) => (
            <Card key={q.id} className="border-4 border-primary rounded-[3rem] overflow-hidden bg-white/10">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="flex gap-4 items-start">
                        <Input 
                          ...
                          className="text-3xl font-black h-20 border-4 border-primary bg-white/10 rounded-[2rem] px-8 focus-visible:ring-0 uppercase placeholder:opacity-10"
                        />
                        <Button variant="ghost" size="icon" onClick={() => removeQuestion(q.id)} className="text-primary hover:bg-primary hover:text-background h-16 w-16 rounded-[1.5rem] border-4 border-primary/10">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid gap-3">
                          {q.options.map((opt, oIdx) => (
                            <div key={oIdx} className="flex gap-4 items-center group">
                              ...
                              <Input 
                                ...
                                className="h-16 border-4 border-primary/20 bg-white/5 rounded-[1.5rem] px-6 focus-visible:ring-0 uppercase font-bold"
                              />
                              <Button 
                                variant="ghost" 
                                size="icon" 
                                ...
                                className="opacity-0 group-hover:opacity-100 h-12 w-12 rounded-xl transition-all border-2 border-primary/10 text-primary hover:bg-primary hover:text-background"
                              >
                          ...
                          <Button 
                            ...
                            className="w-fit h-14 px-8 gap-3 text-primary font-black uppercase tracking-widest border-4 border-primary/10 rounded-[1.5rem] hover:bg-primary hover:text-background transition-all"
                          >
      ...
      <div className="fixed bottom-12 left-1/2 -translate-x-1/2 bg-primary p-3 rounded-[3rem] flex items-center gap-3 border-4 border-primary animate-in slide-in-from-bottom-20 duration-500">
        <Button onClick={() => addQuestion('multiple-choice')} className="rounded-[2rem] h-20 px-8 gap-4 bg-transparent hover:bg-white/10 text-background border-none text-xl font-black uppercase tracking-tighter">
          <ListChecks className="h-8 w-8" /> CHOICE
        </Button>
        <div className="w-1 h-12 bg-background/20 rounded-full" />
        <Button onClick={() => addQuestion('open-text')} className="rounded-[2rem] h-20 px-8 gap-4 bg-transparent hover:bg-white/10 text-background border-none text-xl font-black uppercase tracking-tighter">
          <Type className="h-8 w-8" /> TEXT
        </Button>
        <div className="w-1 h-12 bg-background/20 rounded-full" />
        <Button onClick={() => addQuestion('rating')} className="rounded-[2rem] h-20 px-8 gap-4 bg-transparent hover:bg-white/10 text-background border-none text-xl font-black uppercase tracking-tighter">
